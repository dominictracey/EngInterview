...
public class ReplayAction extends BaseAction implements IAction {
	
	public ReplayAction(int p1, int p2) {
		super(p1,p2);
	}
	
	// use the constructor to set params
	protected ReplayAction() {
	...
	public IState execute(IState state) {
		...
		if (validate(state, p1, p2)) {
			...
			Controller.getInstance().getActionQueue().replay(p1, state);
		}
	...
	// they are just no-ops for now. So size 1; add 1; replay 1; replay 1 >> 1: [X, X] 
	// as the second replay just does a no-op on the first replay
	@Override
